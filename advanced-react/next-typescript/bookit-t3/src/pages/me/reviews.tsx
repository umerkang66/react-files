...
const Reviews: NextPage<Props> = () => {
  ...
  const reviewDeleteMutation = api.review.delete.useMutation({
    onSuccess: () => apiUtils.review.reviewsByUser.invalidate(),
  });

  const { page, take, sort } = getPaginationQueries(router);

  const { data, isFetching } = api.review.reviewsByUser.useQuery(
    { page, take, sort },
    {
      ...
      onSuccess: () => {
        void apiUtils.review.reviewsByUser.prefetch({
          page: page + 1,
          take,
          sort,
        });
...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) => {
  ...
  await api.review.reviewsByUser.prefetch({ page, take, sort });

  // PRE-FETCH THE NEXT PAGE
  // don't use its promise, fetch the next page in the background
  void api.review.reviewsByUser.prefetch({ page: page + 1, take, sort });
